...
def show_confirmation(commit_msg: str, description: str | None, skip_confirm: bool = False) -> bool:
    ...
    def get_terminal_width():
        ...
        try:
            ...
            return min(width, 120)  # Cap at 120 to prevent too wide boxes
        except:
    ...
    def wrap_text(text: str, width: int) -> list[str]:
        ...
        return textwrap.wrap(text, width=width-4, break_long_words=True, break_on_hyphens=True)
    ...
    for line in commit_msg.split('\n'):
        msg_lines.extend(wrap_text(line, term_width))
    
    max_len = max(len(line) for line in msg_lines) if msg_lines else 0
    width = min(max(max_len + 4, 40), term_width)  # Between 40 and terminal width
    ...
    # Description box if present - label on left side of border
    if description:
        print()
        desc_lines = []
