...
def show_confirmation(commit_msg: str, description: str | None, skip_confirm: bool = False) -> bool:
    ...
    print(f"{Fore.CYAN}│ {Fore.YELLOW}Generated by AI{' ' * (preview_width - 15)}{Style.RESET_ALL}{Fore.CYAN}│{Style.RESET_ALL}")
    print(f"{Fore.CYAN}╰{'─' * (preview_width - 2)}╯{Style.RESET_ALL}\n")
    ...
    if len(msg_lines[0]) > 50:
        print(f"\n{Fore.YELLOW}Tip:{Style.RESET_ALL} First line is quite long ({len(msg_lines[0])} chars)")
        print("     Consider keeping it under 50 characters for better readability")
    
    # Show command preview
    print(f"\n{Fore.CYAN}Command preview:{Style.RESET_ALL}")
    commit_preview = f"git commit -m \"{commit_msg}\""
    if description:
        first_desc_line = description.split('\n')[0]
        commit_preview += f' -m "{first_desc_line}..."'  # Fixed f-string syntax
