...
def show_confirmation(commit_msg: str, description: str | None, skip_confirm: bool = False) -> bool:
    ...
    print(f"{Fore.CYAN}│ {Fore.YELLOW}Generated by AI{Style.RESET_ALL}{Fore.CYAN}{' ' * 16}│{Style.RESET_ALL}")
    print(f"{Fore.CYAN}╰───────────────────╯{Style.RESET_ALL}\n")
    
    # Message box - label on left side of border
    msg_lines = commit_msg.split('\n')
    max_len = max(len(line) for line in msg_lines) if msg_lines else 0
    width = max(max_len + 4, 40)  # Minimum width 40 for better appearance
